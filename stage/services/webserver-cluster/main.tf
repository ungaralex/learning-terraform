...
terraform {
  backend "s3" {
    key = "webserver-cluster/stage/cluster/terraform.tfstate"
  }
}
...
resource "aws_security_group" "simple_webserver_alb" {
  name = var.security_group_webserver
  ingress {
    from_port   = 80
    protocol    = "tcp"
    to_port     = 80
...
resource "aws_security_group" "all_egress" {
  name = var.security_group_all_egress
  egress {
    from_port   = 0
    protocol    = "-1"
    to_port     = 0
...
resource "aws_security_group" "webserver" {
  name = var.security_group_webserver_from_alb
  ingress {
